...
class ExamService(
    ...
    fun getExamForStudent(id: String): ExamResponse =
        examRepository.findById(UUID.fromString(id))
            .orElseThrow { RuntimeException("الاختبار غير موجود") }
            .toResponse()
    ...
    fun createExam(request: CreateExamRequest): ExamResponse {
        val schoolClass = classRepository.findById(UUID.fromString(request.classId))
            .orElseThrow { RuntimeException("الفصل غير موجود") }
        ...
        request.questions.forEachIndexed { i, q ->
    ...
    fun submitExam(examId: String, request: SubmitExamRequest): ExamResultResponse {
        ...
        val score    = if (questions.isEmpty()) 0.0
                       else (correct.toDouble() / questions.size) * 100
        val isPassed = score >= exam.passScore
        val student  = userRepository.findById(UUID.fromString(request.studentId))
            .orElseThrow { RuntimeException("الطالب غير موجود") }
    ...
    fun getClassReport(classId: String): ClassReportResponse {
        ...
        val avg      = submissions.mapNotNull { it.score }.average().takeIf { !it.isNaN() } ?: 0.0
        val passRate = if (submissions.isEmpty()) 0.0
                       else submissions.count { it.isPassed == true }.toDouble() / submissions.size * 100
        return ClassReportResponse(
            classId      = classId,
            className    = schoolClass.name,
            studentCount = submissions.map { it.student.id }.distinct().size,
            averageScore = avg,
            passRate     = passRate,
