...
import { Users, BookOpen, FileCheck, TrendingUp, AlertTriangle } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const weeklyActivityData = [
    { day: 'السبت', students: 145, exams: 23 },
    { day: 'الأحد', students: 189, exams: 45 },
    { day: 'الاثنين', students: 234, exams: 67 },
    { day: 'الثلاثاء', students: 198, exams: 34 },
    { day: 'الأربعاء', students: 267, exams: 78 },
    { day: 'الخميس', students: 312, exams: 89 },
    { day: 'الجمعة', students: 89, exams: 12 },
  ]
...
function StatCard({ title, value, icon, color, trend, urgent }: StatCardProps) {
  return (
    <div className={`bg-white rounded-xl border p-5 ${urgent ? 'border-amber-300 shadow-amber-100 shadow-md' : 'border-gray-200'}`}>
      ...
      <div className="mt-2">
        ...
        <p className={`text-xs mt-1 ${urgent ? 'text-amber-600' : 'text-gray-400'}`}>{trend}</p>
      </div>
    </div>
  )
}
